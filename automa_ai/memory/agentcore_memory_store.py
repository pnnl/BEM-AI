...
ACTOR_ID_SOURCES = ("user_id", "session_id")
# The agents send user_id as configurable["actor_id"], so defaulting to "user_id"
# stores memories under the same identity as checkpoints
DEFAULT_ACTOR_ID_SOURCE = "user_id"

# Design notes:
#  - Role-less entries are submitted as synthetic USER turns for conversational
#    extraction. Explicit system roles are preserved, but their content may not
#    produce extracted records depending on the configured strategy.
#  - actor_id is call context (which namespace/stream), not record authorship.
#  - Score checks use `is None`, not `or`, to preserve a real 0.0 score.
...
def _validate_identity_value(field: str, value: Any) -> str:
    ...
    path. Not an authorization check — that is an upstream concern.
    """
...
class AgentCoreMemoryStore(BaseMemoryStore):
    ...
    def from_config(cls, config: dict) -> "BaseMemoryStore":
        ...
        #   region (str, optional)
        #   actor_id_source ("user_id"|"session_id", optional, default "user_id")
        ...
        # actor_id_source is re-validated in __init__, so no check here.
        return cls(
            memory_id=memory_id,
            namespaces=namespaces,
            region=config.get("region"),
            actor_id_source=config.get("actor_id_source") or DEFAULT_ACTOR_ID_SOURCE,
    ...
    def __init__(
        ...
        *,
        ...
        actor_id_source: str = DEFAULT_ACTOR_ID_SOURCE,
        store: Any = None,
        **boto3_kwargs: Any,
    ):
        ...
        if actor_id_source not in ACTOR_ID_SOURCES:
            raise ValueError(
                f"actor_id_source must be one of {ACTOR_ID_SOURCES}, "
                f"got {actor_id_source!r}."
            )
    ...
    def write_memory(self, entries: List[MemoryEntry]) -> None:
        ...
        for entry in entries:
            ...
            actor_id = self._resolve_actor_id(
                user_id=entry.user_id, session_id=entry.session_id
            )
            # PutOp bypasses store.put() to pass actor/session IDs verbatim;
            # store.put() rejects labels with periods, mangling email user IDs.
            ops.append(
                PutOp(
    ...
    def read_memories(
        ...
        **kwargs,
        ...
        """
        # Reject unsupported args before early returns so callers always hear about
        # filters this store cannot honour, regardless of query state.
        #
        # Emptiness, not presence, is the test: DefaultMemoryManager passes
        # metadata to every store as shared plumbing (manager._construct_read_kwargs)
        # and TurnRequest.metadata is a default_factory=dict, so `is not None`
        # would fail every read made through the hook pipeline.
        if metadata:
        ...
        actor_id = self._resolve_actor_id(user_id=user_id, session_id=session_id)
    ...
    def _resolve_actor_id(
        ...
        candidate = user_id if self.actor_id_source == "user_id" else session_id
        if not candidate:
            raise ValueError(
                f"AgentCoreMemoryStore is configured with "
                f"actor_id_source={self.actor_id_source!r}, but no "
                f"{self.actor_id_source} was provided. The actor ID groups written "
                "events; it does not by itself isolate reads (the namespace template "
                "does that), so caller identity must be trusted upstream."
            )
        return _validate_identity_value(self.actor_id_source, candidate)
    ...
    def _render_namespace(
        ...
        for field in fields:
            v = values.get(field)
            # None or "" means absent → skip this namespace rather than rendering
            # a partial path. Other falsy types (0, False, []) are malformed and
            # fall through to _validate_identity_value, which raises.
            if v is None or v == "":
                logger.debug(
    ...
    def _entry_from_item(
        ...
        *,
        ...
        return MemoryEntry(
            ...
            session_id="",
            task_id=None,
            # Not the querying user: a record can live in a shared namespace and
            # the reader is not necessarily its owner. Caller identity is in
            # metadata["agentcore_query_actor_id"].
            user_id=None,
    ...
    def delete_memory(self, record_id: str) -> bool:
        ...
        fact can be re-extracted. Does NOT check namespaces or caller identity —
