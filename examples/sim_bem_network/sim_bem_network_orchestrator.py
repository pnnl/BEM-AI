...
from automa_ai.observability import create_feishu_notifier_from_env

logger = logging.getLogger(__name__)
event_notifier = create_feishu_notifier_from_env()
...
def resolve_backend(provider: str | None, base_url: str | None) -> GenericLLM:
    if provider:
        normalized = provider.strip().lower().replace("_", "-")
        for candidate in GenericLLM:
            if candidate.value == normalized:
                return candidate
        raise ValueError(f"Unsupported model provider: {provider}")

    if base_url:
        normalized_path = urlparse(base_url.strip()).path.rstrip("/")
...
If the user asks to report EUI, energy consumption, or simulation results, include explicit Energy Output Agent task(s) to retrieve the requested outputs after simulations. Output retrieval is allowed and is not considered data analysis.
Do not invent file paths for modified models. For downstream simulation or output tasks after an envelope/lighting modification, refer to the updated model path returned by the previous agent in the blackboard (for example, updated_model_path), instead of hard-coding a new path.
## QUESTION FORMAT
...
planner_model_provider = os.getenv("PLANNER_MODEL_PROVIDER")
planner_backend = resolve_backend(planner_model_provider, planner_model_base_url)
...
model_url = os.getenv("SPECIALIZED_AGENT_MODEL_BASE_URL")
specialized_model_provider = os.getenv("SPECIALIZED_AGENT_MODEL_PROVIDER")
specialized_backend = resolve_backend(specialized_model_provider, model_url)
# Load geometry agent.
# Compute full path to the agent card file
...
env_modeler = AgentFactory(
    ...
    agent_type=GenericAgentType.LANGGRAPH,
    chat_model=specialized_backend,
