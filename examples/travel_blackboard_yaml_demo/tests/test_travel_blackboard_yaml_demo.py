...
from automa_ai.blackboard.backends.local_json import (
    LocalJSONBlackboardStore,
    LocalJSONBlackboardStoreConfig,
)
...
def _build_store(tmp_path: Path) -> LocalJSONBlackboardStore:
    ...
    config = LocalJSONBlackboardStoreConfig(base_dir=str(tmp_path))
    ...
    store.apply_patch(
        ...
        patch=BlackboardPatch(
            ops=[
                {
                    "op": "set",
                    "path": "selection.flight_id",
                    "value": flight_quotes[0]["id"],
                },
                {
                    "op": "set",
                    "path": "selection.hotel_id",
                    "value": hotel_quotes[1]["id"],
                },
            ...
            ops=[
                {
                    ...
                    "value": make_confirmation(
                        "flight", flight_quotes[0]["id"], session_id
                    ),
                },
                {
                    "op": "set",
                    ...
                    "value": make_confirmation(
                        "hotel", hotel_quotes[1]["id"], session_id
                    ),
                },
                {
                    "op": "set",
