...
def main() -> None:
    st.set_page_config(
        page_title="YAML Travel Blackboard Demo", page_icon="✈️", layout="wide"
    )
    st.title("✈️ YAML Travel Booking Blackboard Demo")
    st.caption(
        "YAML-defined LangGraph chat agents with shared blackboard orchestration."
    )
    ...
    with st.sidebar:
        ...
        if st.button(
            "Refresh Blackboard Snapshot", disabled=st.session_state["is_streaming"]
        ):
            st.session_state["bb_snapshot"] = blackboard_read(
                st.session_state["session_id"]
            )
        if st.session_state["is_streaming"]:
            st.info(
                "Streaming is in progress. Snapshot refresh is temporarily disabled to avoid interrupting the stream."
            )
    ...
    if prompt := st.chat_input(
        "Ask the orchestrator to plan and book your trip...",
