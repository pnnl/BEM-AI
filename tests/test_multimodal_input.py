...
def test_input_assembler_builds_multimodal_image_url_content():
    assembler = InputAssembler()
    turn = TurnRequest(
        query="Describe this image",
        context_id="session-1",
...
def _make_url_multimodal_message() -> Message:
    message = Message()
    message.role = Role.ROLE_USER
    message.parts.append(Part(text="Describe this image"))
    message.parts.append(
...
def _make_multimodal_context(message: Message | None = None) -> Mock:
    context = Mock(spec=RequestContext)
    context.message = message or _make_multimodal_message()
...
@pytest.mark.asyncio
async def test_executor_url_image_part_flows_to_multimodal_langgraph_input():
    agent = Mock()
    agent.agent_name = "dummy_agent"
    captured = {}
    ...
    await executor.execute(
        _make_multimodal_context(_make_url_multimodal_message()),
        AsyncMock(spec=EventQueue),
    )
