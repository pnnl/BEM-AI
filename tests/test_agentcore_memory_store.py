...
def test_write_memory_requires_user_id_as_actor() -> None:
    with pytest.raises(ValueError, match="requires a user_id"):
        write_entry(build_store(), user_id=None)


def test_write_memory_uses_user_id_as_actor_id() -> None:
    """The actor is always the user, never the session.

    Scoping memories to a session is done with {session_id} in a namespace
    template, which keeps cross-session queries possible.
    """
    fake = FakeAgentCoreStore()
    write_entry(build_store(fake=fake), user_id="user-1", session_id="s1")
    assert fake.batches[0][0].namespace == ("user-1", "s1")
