...
@pytest.mark.parametrize(
    "checkpointer_type", ["redis_plain", "redis_cluster", "redis_stack"]
)
...
def test_build_checkpointer_uses_redis_cluster_saver(monkeypatch) -> None:
    captured: dict[str, object] = {}

    class FakeRedisClusterSaver:
        def __init__(
            self,
            redis_url: str,
            *,
    ...
    monkeypatch.setattr(agent_factory, "RedisClusterSaver", FakeRedisClusterSaver)

    checkpointer, cleanup = agent_factory._build_checkpointer(
        {"type": "redis_cluster", "redis_url": "localhost:6379"}
    )

    assert isinstance(checkpointer, FakeRedisClusterSaver)
    assert captured["redis_url"] == "redis://localhost:6379"
    assert captured["checkpoint_ttl_seconds"] == DEFAULT_REDIS_CHECKPOINT_TTL_SECONDS
    assert captured["max_checkpoints_per_thread"] == (
        DEFAULT_REDIS_MAX_CHECKPOINTS_PER_THREAD
...
def test_checkpointer_config_rejects_plain_redis_lifecycle_for_redis_stack() -> None:
    with pytest.raises(
        ValueError, match="only supported by redis_plain or redis_cluster"
    ):
        CheckpointerConfig.from_value(
            {
                "type": "redis_stack",
