...
- **Customize health check path**: every A2A server includes a health check endpoint at `/health` by default. Pass `health_check_path="/status"` to `A2AAgentServer` to customize the path. The endpoint returns JSON with `{"status": "healthy"|"unhealthy", "agent": "Agent Name"}`.【F:automa_ai/common/agent_registry.py†L73-L102】
